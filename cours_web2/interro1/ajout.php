                    ...
                    <div class="col-12 col-md-6 py-5 text-center mt-5">
                        <form style="background-color: rgb(251,250,250); border-radius: 20px 20px 20px 20px; box-shadow: rgba(0, 0, 0, 0.4) 0px 30px 90px;" class="row justify-content-center" method="post" action="">
                            ...
                            <div class="input-group mt-1">
                                ...
                                <input style="border-radius: 0px 20px 20px 0px;" name="solde_ajout" type="text" class="form-control" placeholder="montant à ajouter" aria-label="Username" aria-describedby="basic-addon1" required>
                            </div>
...
<?php
    if(isset($_POST['submit'])){
        $id_ajout = $_POST['id'];
        $solde_ajout = $_POST['solde_ajout'];
        $patron1 = new Patron($_POST['id'],$_POST['solde_ajout']);


        $ajout=$patron1->Augmenter_solde($_POST['solde_ajout']);
        // $salaire = ($chiffre * $pourcentage)/100;
        // echo $salaire;
        $insert ="INSERT INTO ajout (designation,montant_ajout) VALUES($solde_ajout,$ajout)";
        ...
        if($execute == true){
            ...
            echo '<div class="row gy-4 align-items-center justify-content-center fs-5 text-warning">'.'Votre solde a augmenté de '.'<strong>'.$_POST['solde_ajout'].'$'.'</strong>'.'</div>';
        }
        else{
            echo "L'enregistrement n'a pas pu être effectué";
        }

        $old=$pdo->query("SELECT salaire FROM employe WHERE id='$id_ajout'");

        if ($d = $old->fetch()) {
            $v = $d['salaire'] + $solde_ajout;
            # code...
            // $entree_produit= new Produit($_POST['id_produit'],null,null,null,$_POST['Qnte_entree']);
            $augmenter_stock=$pdo->query("UPDATE employe set salaire = '$v' WHERE id='$id_ajout' ");
